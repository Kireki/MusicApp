...
namespace GetYourJam_Working_Title_.Controllers
{
    [Authorize]
    public class AccountController : Controller
    {
        ...
        //
        // GET: /Account/FbAuthenticationToken
        ...
        public async Task<ActionResult> FacebookArtistLikes()
        {
            ...
            dynamic fbArtistLikes = FbClient.Get("/v2.2/me/music");
            var fbLikedArtists = new List<FacebookViewModel>();
            foreach (var artistLike in fbArtistLikes.data)
            {
                fbLikedArtists.Add(new FacebookViewModel()
                {
                    Id = artistLike.id,
                    Name = artistLike.name,
                    Image = @"https://graph.facebook.com/" + artistLike.id + "/picture?type=large"
                });
            }

            //getting Last.fm data
            //TODO:Optimize this
            HashSet<string> similarArtists = new HashSet<string>();
            foreach (var artist in fbLikedArtists)
            {
                Debug.WriteLine(String.Format("Looking up artists similar to {0}…", artist.Name));
                Task<HashSet<string>> getSimilars = GetSimilarArtists(artist.Name);
                HashSet<string> similars = await getSimilars;
                similarArtists.UnionWith(similars);
                similarArtists.Add(artist.Name);
            }

//            var client = new RestClient("http://ws.audioscrobbler.com/");
//            var request = new RestRequest("2.0/?method=artist.getsimilar&artist={artistName}&api_key=dc801c2df9fdf6605ff5de6ada4ecca5&format=json", Method.GET);
...
//            Debug.WriteLine(response.Content);
            ...
            {
                Debug.WriteLine(String.Format("Looking up a user ID for {0}…", artist));
                var getUserIdTask = GetUserId(artist);
            ...
            foreach (var id in userIds)
            {
                ...
                var getUserTracksTask = GetUserTracks(id);
                var userTracks = await getUserTracksTask;
                tracks.AddRange(userTracks);
            }

//            var client = new RestClient("http://api.soundcloud.com/");
//            var request = new RestRequest("users.json");
//            request.AddQueryParameter("q", "white%20lies");
//            request.AddParameter("client_id", "f31b372d38cde9769c9e54a8e29bc8aa");
...
//            Debug.WriteLine(response.Content);
        ...
        private async Task<HashSet<string>> GetSimilarArtists(string artistName)
        {
            HttpClient client = new HttpClient();
            string url = String.Format("http://ws.audioscrobbler.com/2.0/?method=artist.getsimilar&artist={0}&api_key=dc801c2df9fdf6605ff5de6ada4ecca5&format=json", HttpUtility.UrlEncode(artistName));
            HttpResponseMessage response = await client.GetAsync(url);
            var responseText = await response.Content.ReadAsStringAsync();
            LastFmRootObject similarArtists = JsonConvert.DeserializeObject<LastFmRootObject>(responseText);
